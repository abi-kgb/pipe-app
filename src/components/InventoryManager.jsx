...
import { X, Save, RefreshCw, AlertCircle, Package, Search, Database, CheckCircle2 } from 'lucide-react';
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                fetchInventory(); // Refresh to get server-side updates/sorting
            } else {
            ...
            <div className="relative w-full max-w-5xl bg-slate-900 border border-slate-800 rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col max-h-[90vh]">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">
                                Connection: {error ? 'Disconnected' : isLoading ? 'Connecting...' : 'Active'}
                            </p>
                        </div>
                    </div>
                            ...
                            <table className="w-full text-left text-sm border-collapse">
                                <thead className="bg-slate-900/80 sticky top-0 z-10 backdrop-blur-sm">
                                    <tr className="border-b border-slate-800">
                                        ...
                                        <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-widest text-[10px] text-right">Adjust Stock</th>
                                    </tr>
                                </thead>
                ...
                <div className="p-6 border-t border-slate-800 bg-slate-900/50 flex items-center justify-between">
                    ...
                    <div className="flex gap-4">
                        <button
                            onClick={onClose}
                            className="px-6 py-2.5 rounded-2xl border border-slate-800 text-slate-500 font-bold text-xs uppercase tracking-widest hover:bg-slate-800 hover:text-white transition-all"
                        >
                            Close
                        </button>
                        <button
