...
const VIEW_CONFIGS = {
  iso: { label: '3D Isometric', labelColor: 'bg-slate-900', labelTextColor: 'text-white', camera: 'perspective', defaultPos: [30, 30, 30], defaultFov: 50, defaultUp: [0, 1, 0] },
...
const ViewportCameraControls = ({ viewMode, setResetHandler, setZoomHandlers, selectedIds, components, isLocked }) => {
  ...
  const handleReset = useCallback((fitToComponents = false, animate = true) => {
    ...
    if (fitToComponents && components && components.length > 0) {
      ...
      const fitRadius = Math.max(maxDim * 0.7, 5);
      ...
      if (config.camera === 'ortho') {
        ...
        // ISO view from default position relative to center
        const defaultOffset = new THREE.Vector3().fromArray(config.defaultPos);
        controlsRef.current.setLookAt(
          center.x + defaultOffset.x, center.y + defaultOffset.y, center.z + defaultOffset.z,
...
const Scene3D = forwardRef(function Scene3D({
  ...
  useImperativeHandle(ref, () => ({
    ...
    captureViews: () => new Promise(async (resolve) => {
      // 1. Snap all cameras instantly to frame content
      Object.keys(VIEW_CONFIGS).forEach(viewId => {
        captureHandlers.current[viewId]?.(true, false);
      });

      // 2. Wait for initial render of hidden viewports
      await new Promise(r => setTimeout(r, 1000));

      const images = {};
      console.log(`Pipe3D Export: Starting async view capture (yielding for UI)...`);

      // 3. Capture one by one, yielding to the main thread to prevent UI freezing
      const viewIds = Object.keys(VIEW_CONFIGS);
      for (const viewId of viewIds) {
        // Yield to browser for UI updates/input processing
        ...
        if (!wrapper) {
          console.warn(`Pipe3D Export: No wrapper for ${viewId}`);
          continue;
        }

        const canvas = wrapper.querySelector('canvas');
        if (!canvas) {
          console.warn(`Pipe3D Export: No canvas for ${viewId}`);
          continue;
        }

        try {
          // Use JPEG with 0.85 quality for faster processing and lower memory overhead than PNG
          const data = canvas.toDataURL('image/jpeg', 0.85);
          if (data.length < 1000) {
            console.warn(`Pipe3D Export: ${viewId} capture failed or blank`);
          } else {
            console.log(`Pipe3D Export: Captured ${viewId} (${(data.length / 1024).toFixed(1)} KB)`);
          }
          images[viewId] = data;
        } catch (e) {
          console.error(`Pipe3D Export: Capture error for ${viewId}:`, e);
        }
      }

      if (Object.keys(images).length === 0) {
        console.error('Pipe3D Export: No images captured!');
      }
      resolve(images);
    })
  }), [components]);
    ...
    >
      ...
      <div
        style={{
          ...
          top: 0,
          left: 0,
          zIndex: -1000, // Buried behind everything
          opacity: 0.05,  // Just enough to keep browser interested in rendering
          pointerEvents: 'none',
          width: '1280px', // Reduced from 2400px for performance
          height: '960px',
          overflow: 'hidden'
        }}
        data-html2canvas-ignore="true"
      >
        {isCapturing && Object.keys(VIEW_CONFIGS).map(viewId => (
          <div key={`capture-${viewId}`} id={`capture-canvas-${viewId}`} style={{ width: '1280px', height: '960px', backgroundColor: 'white' }}>
